    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            if ($uploaded_file && is_uploaded_file($uploaded_file)) {
				...
				$tfilename=basename($file_path);
				$lat = 0; 
		        $lng = 0;
				/*$c = getCoordinates($uploaded_file); 
		        $lat = $c[0]; 
		        $lng = $c[1]; */
				
				
				$mdate = date('Y-m-d H:i:s');
				$sqlstr="insert into  mhv.tblimages(albumid,filename,location,lat,lng,uploaddate) values(0,'$tfilename','../vendor/multiUpload/server/php/files/','$lat','$lng','$mdate')";
            ...
            if ($success) {
                foreach($this->options['image_versions'] as $version => $options) {
                    if (!empty($version)) {
                        ...
                        if (is_file($file)) {
                            unlink($file);
							$sqlstr="delete from  mhv.tblimages where filename='$file_name'";
							mysql_query($sqlstr);
                        }
                    }
                }
