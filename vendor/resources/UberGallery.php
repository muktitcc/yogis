<?php
require_once( "../rmt/common.inc_.php" );
/**
...
class UberGallery {
    ...
    protected $_imgPerPage  = 25;
    protected $_thumbSize   = 100;
    protected $_threshold   = 10;
    protected $_themeName   = 'uber-orange';
    protected $_page        = 1;
    ...
    function __construct() {
        ...
        // Set class directe;ory constant
        if(!defined('__DIR__')) {
     ...
     * @access public
     */
    public function createGallery($directory, $relText = 'colorbox',$filename) {
        
        $gallery = $this->readImageDirectory($directory,$filename,$page);
        /*
     ...
     * @access public
     */
    public function readImageDirectory($directory,$filename,$page) {
       // $this->_page=$page;
        // Set relative image directory
        $this->setRelativeImageDirectory($directory,$page);
        ...
        if (file_exists($this->_index) && (time() - filemtime($this->_index)) / 60 < $this->_cacheExpire) {
            ...
            $dirArray = $this->_readDirectory($directory,true,$filename,$page);
            ...
            foreach ($dirArray as $key => $image) {                        
				...
				$downloadPath = RMTUPLOADEDDATA_DIR .'gallery-images-original/' . $key;
                ...
                $galleryArray['images'][htmlentities(pathinfo($image['real_path'], PATHINFO_BASENAME))] = array(
					...
					'file_info' =>$this->getImageInfo($key),
					'file_height'=>$this->getImageHeight($key),
					'file_width'=>$this->getImageWidth($key),
                    'file_path'    => htmlentities($relativePath),
					'download_path'    => htmlentities($downloadPath),
            ...
            $galleryArray['stats'] = $this->_readGalleryStats($this->_readDirectory($directory,false,$filename,$page),$page);
            
            // Add gallery paginator to the gallery array
            $galleryArray['paginator'] = $this->_getPaginatorArray($page, $galleryArray['stats']['total_pages']);
     ...
     * @access public
     */
    public function setRelativeImageDirectory($directory,$page) {
        ...
        if ($this->_imgPerPage < 1) {
            ...
            $this->_index = $this->_cacheDir . '/' . md5($directory) . '-' . $page . '.index';            
        }
        //print_r($this);
        return $this;
    }
     ...
     * @access protected 
     */
    protected function _readDirectory($directory, $paginate = true,$filename,$page) {
        ...
        if ($paginate == true && $this->_imgPerPage > 0) {
            $dirArray = $this->_arrayPaginate($dirArray, $this->_imgPerPage, $page);
        }
     ...
     * @access protected
     */
    protected function _readGalleryStats($array,$page) {
	...
	protected function getImageInfo($image){
		$message ='';
	$result = mysql_query("SELECT * FROM mhv.tblimages where  filename='$image'") or die(mysql_error());	
	...
	$filesize= $row['filesize'] . ' MB';	
	  $message = '
<table border="1">
<tr><td align="center" colspan="4">Image Information</td></tr>
	  <tr><td >Tag</td><td>'.$mtag .'</td></tr>
	  <tr><td>Dimention(HXW)</td><td>'.$height .'X' .$width .'</td></tr>
...
';
	 ...
	 //<tr><td>Title</td><td>'.$mtitle .'</td></tr>
	  //<tr><td>Description</td><td>'.$mdesc .'</td></tr>
return $message;
...
return $height;
	...
	protected function getImageWidth($image){
	$message ='';
	$result = mysql_query("SELECT * FROM mhv.tblimages where  filename='$image'") or die(mysql_error());	
	$row = mysql_fetch_array($result);
	$width= $row['width'];
